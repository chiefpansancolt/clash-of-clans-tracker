...
export const LabAssistantCard = ({ hv, level, assignment, thLevel, onLevelChange, onAssignmentChange }: Props) => {
  ...
  const activeUpgrades = getActiveResearchUpgrades(hv);

  const savedMs = currentLevelData ? currentLevelData.workRate * 3_600_000 : 0;

  const assignedLabel = assignment
    ? activeUpgrades.find((o) => JSON.stringify(o.target) === JSON.stringify(assignment.target))?.label ?? "Assigned (research done)"
    : null;
    ...
    <div className={`rounded-xl border border-secondary/80 bg-primary overflow-hidden ${locked ? "opacity-50" : ""}`}>
      <div className="flex items-center gap-3 px-4 py-3 border-b border-secondary/80">
        <div className="relative h-10 w-10 shrink-0">
          <Image
            src={toPublicImageUrl("images/home/other/helpers/lab-assistant/normal.png")}
            alt="Lab Assistant"
            fill
            sizes="40px"
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-extrabold text-white">Lab Assistant</p>
          {locked && (
        ...
        <>
          ...
          <div className="px-4 py-3">
            ...
            {activeUpgrades.length === 0 ? (
              <p className="text-[11px] text-white/80">No active lab research to assign.</p>
            ) : (
              <>
                <select
            ...
            {assignedLabel && (
              <div className="mt-3 rounded-lg border border-accent/40 bg-accent/8 px-3 py-2">
                ...
                <p className="text-[10px] text-white/80">{assignment!.mode === "once" ? "Clears after 1 use" : "Stays until research completes"}</p>
                <button
                  type="button"
