...
import { getTrapsAtTH } from "@/lib/utils/massEditHelpers";
...
const TrapsUpgradePage = () => {
  ...
  const buildings = useMemo(() => getTrapsAtTH(thLevel), [thLevel]);
    ...
    <div className="flex h-full flex-col overflow-hidden">
      <div className="shrink-0 bg-highlight px-4 py-3">
        <div className="flex items-baseline gap-3">
          <h1 className="text-xl font-extrabold text-gray-900">Traps</h1>
        ...
        <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
          {buildings.map((b) => {
            const instances = Array.from({ length: b.instanceCount }, (_, i) => {
              const inst = (hv.traps[b.id] ?? [])[i] ?? { level: 0 };
              return {
            ...
            return (
              <UpgradeRow
                ...
                onStartUpgrade={(idx, step, builderId) =>
                  save(startBuildingUpgrade(hv, "traps", b.id, idx, step, builderId))
                }
                onFinishUpgrade={(idx) =>
                  save(finishBuildingUpgrade(hv, "traps", b.id, idx))
                }
                onCancelUpgrade={(idx) =>
                  save(cancelBuildingUpgrade(hv, "traps", b.id, idx))
                }
                onAdjustUpgrade={(idx, finishesAt) =>
                  save(adjustBuildingUpgrade(hv, "traps", b.id, idx, finishesAt))
                }
              />
            );
          })}
...
export default TrapsUpgradePage;
